...
let camera_offset;
let camera_velocity;
let currentWorldMode = "overworld";
...
function windowResized() {
  ...
  // Optional: re-center camera if you want
  camera_offset = new p5.Vector(-width / 2, height / 2);
}

function keyPressed() {
  if (key === '1') {
    currentWorldMode = "overworld";
    rebuildWorld(document.querySelector("#canvas-container input").value);
  } else if (key === '2') {
    currentWorldMode = "underwater";
    rebuildWorld(document.querySelector("#canvas-container input").value);
  } else if (key === '3') {
    currentWorldMode = "civilization";
    rebuildWorld(document.querySelector("#canvas-container input").value);
  }
}
...
function draw() {
  ...
  if (currentWorldMode == "underwater") { // Only if underwater mode
    noStroke();
    fill(0, 100, 255, 50); // transparent light blue
    rect(-width, -height, width * 3, height * 3); // cover the screen
  }
...
function drawOverworldTile([world_x, world_y], [camera_x, camera_y]) {
...
function drawTile([world_x, world_y], [camera_x, camera_y]) {
  ...
  if (currentWorldMode === "overworld") {
    if (window.p3_drawTile) {
      window.p3_drawTile(world_x, world_y, -screen_x, screen_y);
    }
  } else if (currentWorldMode === "underwater") {
    if (window.p3_drawUnderwaterTile) {
      window.p3_drawUnderwaterTile(world_x, world_y, -screen_x, screen_y);
    }
  } else if (currentWorldMode === "civilization") {
    if (window.p3_drawCivilizationTile) {
      window.p3_drawCivilizationTile(world_x, world_y, -screen_x, screen_y);
    }
  }
  pop();
}
